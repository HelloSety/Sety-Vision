# AI Audit — Sety Studio

Framework para conduzir auditorias comerciais e de automação/IA em clientes da Sety Studio, com foco em processos comerciais. Gera relatórios detalhados e apresentações executivas de forma semi-automatizada usando LLMs.

**Onde isso se encaixa:** é a etapa **pós-venda** e aprofundada, contraparte do `/diagnostico-comercial` (skill na raiz do repo, usada **pré-venda** pra prospecção rápida via pesquisa pública). Use este framework quando o cliente já fechou e o objetivo é um dossiê completo baseado em entrevistas reais (discovery calls, mapeamento de processo).
...
Segue a convenção do `CLAUDE.md` raiz — sempre dentro de `clientes/<nome-do-cliente>/`, nunca solta no root:
...
Gerar com: `python execution/create_client_structure.py "<Nome do Cliente>" --base-path "../clientes/<nome-do-cliente>"`
