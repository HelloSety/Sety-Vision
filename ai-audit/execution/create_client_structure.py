...
def create_client_structure(client_name: str, base_path: str = "."):
    ...
    Convenção Sety Studio: chame com base_path="clientes/<nome-do-cliente>" para que a
    estrutura fique em `clientes/<nome-do-cliente>/ai-audit/`, seguindo o padrão do
    CLAUDE.md raiz. Sem --base-path, cria "./ai-audit" (uso standalone fora do repo principal).
    """
    root_folder = os.path.join(base_path, "ai-audit")
...
""",
            "AI Audit/VALUE Scoring Matrix.csv": "Processo,Categoria,Impacto (1-5),Esforco (1-5),Prioridade,Notas\n",
            "AI Audit/Final Audit Report.md": "# Relatorio Final de Auditoria\n\n*Pendente - conclusao das etapas do audit.*"
        }
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base-path", default=".",
                         help="Pasta onde criar 'ai-audit/' (ex: clientes/<nome-do-cliente>)")
    args = parser.parse_args()

    create_client_structure(args.name, args.base_path)
