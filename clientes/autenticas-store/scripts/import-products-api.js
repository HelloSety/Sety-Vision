// Importa os 25 produtos reais da Autênticas Store na Shopify via Admin API
// (productSet + productCreateMedia + productVariantAppendMedia), com SEO por produto.
...
const IMAGE_BASE_URL = 'https://autenticas-store-sety.vercel.app';
...
const VARS_TMP = path.join(os.tmpdir(), 'as-vars.json');
...
function seoTitle(p) {
  const t = `${p.name} — ${CATEGORY_LABEL[p.category] || 'Moda'} | Autênticas Store`;
  return t.length > 70 ? `${p.name} | Autênticas Store` : t;
}

function seoDescription(p) {
  const t = `${p.tagline} Frete grátis para todo o Brasil e 5% de desconto no Pix.`;
...
let list = PRODUCTS;
if (onlyHandle) list = PRODUCTS.filter((p) => handleize(p.id) === onlyHandle);

let ok = 0;
let fail = 0;

list.forEach((p, idx) => {
  const handle = handleize(p.id);
  ...
  const variants = p.variants.map((v, i) => ({
    sku: `AS-${handle.toUpperCase()}-${String(i + 1).padStart(2, '0')}`,
  ...
  const sizesLine = p.sizes && p.sizes.length && p.sizes[0].indexOf('Único') !== 0
    ? `<p>Tamanhos disponíveis: ${p.sizes.join(', ')}.</p>` : '';

  const productInput = {
    title: p.name,
    handle,
    descriptionHtml: `<p>${p.tagline}</p>${sizesLine}<p>Produto Autênticas Store. Estoque próprio, envio rápido para todo o Brasil.</p>`,
    vendor: 'Autênticas Store',
    productType: CATEGORY_LABEL[p.category] || 'Moda',
    tags,
    status: 'ACTIVE',
