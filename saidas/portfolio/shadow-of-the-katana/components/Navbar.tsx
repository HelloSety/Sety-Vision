      ...
      >
        ...
        >
          <button
            data-cursor
            onClick={() => go("hero")}
            className="font-serif text-lg italic tracking-tight text-white"
          >
          ...
          <nav className="hidden items-center gap-8 lg:flex">
            {LINKS.slice(0, -1).map((link) => (
              <button
                ...
                data-cursor
                onClick={() => go(link.id)}
                className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-white/75 transition-colors hover:text-white"
              >
                {link.label}
              </button>
            ))}
          ...
          <button
            data-cursor
            onClick={() => go("contact")}
            className="hidden rounded-full bg-white px-5 py-2.5 font-mono text-xs font-bold uppercase tracking-[0.15em] text-ink transition-transform hover:scale-105 lg:inline-flex"
          >
            Fale com a Sety
          </button>
