...
export default function Navbar() {
  ...
  return (
    <>
      <header
        className={`fixed inset-x-0 top-0 z-500 transition-all duration-500 ${scrolled ? "py-3" : "py-6"}`}
      >
        <div
        ...
        >
          <button data-cursor onClick={() => go("hero")} className="font-black text-lg tracking-tight text-ink">
          ...
          <nav className="hidden items-center gap-8 lg:flex">
            {LINKS.slice(0, -1).map((link) => (
              <button
                ...
                data-cursor
                onClick={() => go(link.id)}
                className="font-mono text-xs font-semibold uppercase tracking-[0.15em] text-mist transition-colors hover:text-ink"
              >
                {link.label}
              </button>
            ))}
          ...
          <button
            data-cursor
            onClick={() => go("contact")}
            className="hidden rounded-full bg-ink px-5 py-2.5 text-xs font-bold uppercase tracking-[0.12em] text-white transition-transform hover:scale-105 lg:inline-flex"
          >
            Reserve
          </button>
          ...
          <button
            ...
            aria-label="Menu"
          >
            <motion.span animate={open ? { rotate: 45, y: 4 } : { rotate: 0, y: 0 }} className="h-px w-6 bg-ink" />
