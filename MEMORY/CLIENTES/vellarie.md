...
## 2026-09-05 — DEPLOY feito (tema não publicado)

**`shopify theme push` FUNCIONOU** — usa o token Admin (`store auth`, escopo `write_themes`), diferente de `theme dev`/`theme list` que continuam bloqueados ("conta não é staff da dev store"). Tema **"VELLARIE — SIGNATURE V6" (#163248799875)** no ar como **NÃO PUBLICADO**, `theme check` local 0 erros e validação do servidor Shopify 0 erros. Tema ao vivo (**Horizon**, MAIN) intacto.
- Preview (precisa estar logado no admin da loja): `https://4zevyg-1g.myshopify.com?preview_theme_id=163248799875`
- Editor: `https://4zevyg-1g.myshopify.com/admin/themes/163248799875/editor`
- **Órfãos a apagar no admin** (tentativas de push que falharam antes dos fixes): temas #163248636035 e #163248701571 (mesmo nome). `shopify theme delete` foi barrado pelo guard da sessão.
- **Fixes que destravaram o push** (a validação server-side da Shopify é mais rígida que o `theme check`): `"default": ""` em setting `text`/`textarea` é rejeitado (removido de featured-collection `eyebrow`, hero `cta_label`/`cta2_label`, promo-banner `end_date`, testimonials `review_count`/`note`); label de `option` de `select` > 50 chars é rejeitado (encurtado o do hero `text_source`).
- **Runtime QA (clicar o fluxo de compra no tema renderizado) NÃO foi possível** — `theme dev` bloqueado, preview de tema não publicado exige sessão admin no navegador que eu não tenho aqui. Seven abre o preview logado e testa Home→Product→Variant→Add to Cart→Cart→Checkout.
