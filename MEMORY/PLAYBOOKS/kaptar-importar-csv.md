...
- **Não reenviar pro mesmo contato = nativo.** Antes de cada envio o motor roda `podeEnviarPara(livro, telefone)`: se está em `numero.json > naoPerturbe` ou recebeu msg nos últimos **90 dias** (`CARENCIA_DIAS`), **pula** (não para). Vale campanha manual e `filaViva` do autopilot. `numero.json` nasce no 1º envio bem-sucedido.
